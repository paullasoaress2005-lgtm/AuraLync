...
export default async function RecoveryCodePage({ searchParams }: PageProps) {
  ...
  const message =
    params?.error === "expired"
      ...
      : params?.error
        ? attempts >= 2
          ? "Codigo incorreto. Esta e a ultima tentativa antes do suporte."
          : "Codigo incorreto. Confira o WhatsApp e tente novamente."
        : null;
  ...
  return (
    <main className="min-h-screen bg-[#f8fbfb] px-5 py-8 text-[#102f36]">
      <section className="mx-auto mt-10 w-full max-w-[520px] rounded-xl border border-[#dfe8e7] bg-white p-6 shadow-[0_24px_70px_rgba(15,60,67,0.08)]">
        ...
        <h1 className="mt-6 text-2xl font-semibold">Digite o codigo recebido</h1>
        <p className="mt-2 text-sm leading-6 text-[#6f8588]">
          Enviamos um codigo numerico para o WhatsApp cadastrado, se o contato existir no CRM.
        </p>
